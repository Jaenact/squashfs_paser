...
def parse_superblock(f):
    """
    슈퍼블록 96바이트 읽고, 파싱된 정보를 딕셔너리로 반환환
    """
    print("--- 1. 슈퍼블럭 파싱 ---")
    f.seek(0)
    sb = {} 
    ...
    sb['compression_id'] = int.from_bytes(f.read(2), 'little')
...
def read_metablock(f, table_start_offset, block_ref):
    """
    지정된 위치의 메타데이터를 읽고 XZ 압축을 해제하여 원본 데이터 반환
    """
...
def read_fragment_table(f, sb):
    ...
    for pointer in metablock_pointers:
        ...
        is_compressed = not (header_val & 0x8000)
        compressed_data = f.read(data_size)
        data = lzma.decompress(compressed_data) if is_compressed else compressed_data
...
def extract_entry(f, sb, fragment_table, inode_ref, current_path, output_dir):
    ...
    # --- 1. 아이노드(Inode) 데이터 확보 ---
    # (설명: inode_ref는 "블록 위치(상위 48비트) + 블록 내 오프셋(하위 16비트)" 정보입니다.)
    inode_block_start_offset = (inode_ref >> 16) & 0xFFFFFFFF
    inode_offset_in_block = inode_ref & 0xFFFF
    
    inode_data = read_metablock(f, sb['inode_table_start'], inode_block_start_offset)
    ...
    if inode_type == 1 or inode_type == 8:
        ...
        # 디렉토리 처리 함수 호출 -> 디렉토리 안에 또 파일이 있을 수 있기에 재귀적 호출
...
def process_directory(f, sb, fragment_table, inode_type, inode_body_offset, inode_data, current_path, output_dir):
    ...
    # --- 2. 디렉토리 테이블 블록 읽기 ---
    dir_data = read_metablock(f, sb['directory_table_start'], dir_block_start)
    bytes_read = 0
    current_entry_offset = dir_offset
    
    # --- 3. 자식 엔트리 순회 (while 루프) ---
    # (설명: 디렉토리 데이터는 [헤더+엔트리묶음]이 반복되는 구조)
    ...
    with open(full_local_path, 'wb') as out_f:
        ...
        for i in range(block_count):
            ...
            if block_size_on_disk == 0:
                ...
                decompressed_data = lzma.decompress(compressed_data) if is_compressed else compressed_data
        ...
        if has_fragment:
            ...
            if fragment_size > 0:
                ...
                decompressed_frag_block = lzma.decompress(compressed_frag_block) if is_compressed else compressed_frag_block
...
def process_symlink(inode_type, inode_body_offset, inode_data, full_local_path, indent, display_name):
    ...
    try:
        ...
        print(f"  {indent}[경고] 심볼릭 링크 생성 실패 (권한 문제일 수 있음)")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print(f"사용법: python {sys.argv[0]} <squashfs_파일>")
        print("결과는 ./output/ 폴더에 저장됩니다.")
        sys.exit(1)
